  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="普通积分" prop="ordIntegral">
        <el-input
          v-model="queryParams.ordIntegral"
          placeholder="请输入普通积分"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="奖励积分" prop="rewIntegral">
        <el-input
          v-model="queryParams.rewIntegral"
          placeholder="请输入奖励积分"
          clearable
          @keyup.enter.native="handleQuery"
        />
...
<script>
import { listLog, getLog, delLog, addLog, updateLog } from "@/api/biz/log";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除积分变动日志编号为"' + ids + '"的数据项？').then(function() {
        return delLog(ids);
      }).then(() => {
        this.getList();
