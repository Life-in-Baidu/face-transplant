<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="用户身份" prop="identity">
        <el-select v-model="queryParams.identity" placeholder="请选择用户身份" clearable>
          <el-option
            v-for="dict in dict.type.my_identity_type"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="扩展信息字段" align="center" prop="extraInfo" />
      <el-table-column label="用户身份" align="center" prop="identity">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.my_identity_type" :value="scope.row.identity"/>
        </template>
      </el-table-column>
...
<script>
import { listUser, getUser, delUser, addUser, updateUser } from "@/api/biz/user";

export default {
  name: "User",
  dicts: ['my_identity_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
